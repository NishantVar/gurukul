...
|---------|-------------|
| "Post a message in #general saying hi" | Agent sends the message as your bot |
| "What were the last 5 messages in #general?" | Agent reads and summarizes them |
...
**Important:** Copy the token your agent shows you and save it somewhere safe — a note, a password manager, anywhere you won't lose it. Discord only shows it once. If you lose it, you'll have to generate a new one.

**Verify:** Your agent shows you a long string of characters (the token) and confirms Message Content Intent is enabled.

### Step 4: Invite the bot to your server

The bot exists on Discord's side, but it hasn't joined your server yet. Your agent will generate an invite link with full admin permissions and use it.

> Generate an invite URL for this bot with Administrator permissions, then open it and add the bot to The New Order server.

Administrator permission lets the bot do everything on the server. When the invite page asks which server to add the bot to, make sure you select **The New Order**.
...
> Send a message in the #general channel on The New Order Discord server that says "Hello from [your name]'s coding agent! Bot setup complete."

Open Discord and check #general on The New Order. If the message is there, posted by your bot, everything is working. Try one more:
...
The bot doesn't have permission in that specific channel. Either it wasn't given Administrator permissions during Step 4, or the channel has a permission override. Ask your agent: "Check what permissions my bot has on this server." If the bot lacks Administrator permissions, go back to Step 4 and re-invite it with the correct permissions — the new invite will update the existing bot's permissions.
...
- **Catch up on conversations:** "Summarize what's been happening in #general today." Your agent reads and condenses the messages so you can catch up without scrolling.
...
- **Post from your agent:** "Send a message in #general asking if anyone wants to pair up on the next project." Your bot posts on your behalf — everyone sees it's from your named bot.
